...
function f1(y, x, u, w)
    ...
    yθ = y[nx .+ (1:nθ)]
    [
        double_integrator_2D(h, y_di, x_di, u_ctrl, w);
        yθ - θ;
    ]
end
...
vis = Visualizer()
open(vis) 
visualize_double_integrator_2D!(vis, [trajopt.x]; Δt=h, xT=[xT])
...
for t = 1:T-1 
    push!(u_hist, policy(θ_sol, x_hist[end], [3.0, 1.0, 0.0, 0.0]))
